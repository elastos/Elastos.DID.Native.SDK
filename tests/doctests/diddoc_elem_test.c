...
static void test_diddoc_get_publickey(void)
{
    ...
    for (j = 0; j < 4; j++) {
        ...
        for (i = 0; i < size; i++) {
            ...
            CU_ASSERT_EQUAL(1, DID_Equals(did, &(id->did)));
        ...
        primaryid = DIDURL_NewFromDid(did, "primary");
...
static void test_diddoc_add_publickey(void)
{
    ...
    for (j = 0; j < 4; j++) {
        ...
        DIDURL *id2 = DIDURL_NewFromDid(did, "test2");
...
static void test_diddoc_remove_publickey(void)
{
    ...
    for (j = 0; j < 4; j++) {
        ...
        keyid = DIDURL_NewFromDid(did, "notExistKey");
...
static void test_diddoc_get_authentication_key(void)
{
    ...
    for (j = 0; j < 4; j++) {
        ...
        for (i = 0; i < size; i++) {
            ...
            CU_ASSERT_EQUAL(1, DID_Equals(did, PublicKey_GetController(pk)));
        ...
        id = DIDURL_NewFromDid(did, "primary");
        CU_ASSERT_PTR_NOT_NULL(id);
        ...
        keyid = DIDURL_NewFromDid(did, "key3");
        CU_ASSERT_PTR_NOT_NULL(keyid);
...
static void test_diddoc_remove_authentication_key(void)
{
    ...
    for (j = 0; j < 4; j++) {
        ...
        DIDURL *id3 = DIDURL_NewFromDid(did, "key2");
        CU_ASSERT_PTR_NOT_NULL(id3);
...
static void test_diddoc_get_authorization_key(void)
{
    ...
    for (j = 0; j < 4; j++) {
        ...
        for (i = 0; i < size; i++) {
            ...
            CU_ASSERT_EQUAL(0, DID_Equals(did, PublicKey_GetController(pk)));

            CU_ASSERT_TRUE(!strcmp(id->fragment, "recovery"));
        }

        // AuthorizationKey getter
        keyid = DIDURL_NewFromDid(did, "recovery");
        CU_ASSERT_PTR_NOT_NULL(keyid);
...
static void test_diddoc_remove_authorization_key(void)
{
    ...
    for (j = 0; j < 4; j++) {
        ...
        DIDURL *recoveryid = DIDURL_NewFromDid(did, "recovery");
        CU_ASSERT_PTR_NOT_NULL(recoveryid);
...
static void test_diddoc_get_credential(void)
{
    ...
    for (j = 0; j < 4; j++) {
        ...
        // Credential getter.
        DIDURL *profileid = DIDURL_NewFromDid(did, "profile");
        CU_ASSERT_PTR_NOT_NULL(profileid);
        vc = DIDDocument_GetCredential(doc, profileid);
        CU_ASSERT_PTR_NOT_NULL(vc);
        CU_ASSERT_EQUAL(1, DIDURL_Equals(profileid, Credential_GetId(vc)));
        ...
        CU_ASSERT_EQUAL(1,DIDURL_Equals(Credential_GetId(vcs[0]), profileid));
...
static void test_diddoc_add_credential(void)
{
    ...
    for (j = 1; j < 4; j++) {
        ...
        DIDURL *id = DIDURL_NewFromDid(did, "passport");
        CU_ASSERT_PTR_NOT_NULL(id);
...
static void test_diddoc_add_selfclaimed_credential(void)
{
    ...
    for (j = 0; j < 4; j++) {
        ...
        DIDURL *credid = DIDURL_NewFromDid(did, "passport");
        ...
        CU_ASSERT_PTR_NOT_NULL(vc);
        CU_ASSERT_EQUAL(1, Credential_IsSelfProclaimed(vc));
...
static void test_diddoc_get_service(void)
{
    ...
    for (j = 0; j < 4; j++) {
        ...
        DIDURL *openid = DIDURL_NewFromDid(did, "openid");
        CU_ASSERT_PTR_NOT_NULL(openid);
        service = DIDDocument_GetService(doc, openid);
        CU_ASSERT_PTR_NOT_NULL(service);
        CU_ASSERT_EQUAL(1,DIDURL_Equals(openid, Service_GetId(service)));
        ...
        DIDURL *vcrid = DIDURL_NewFromDid(did, "vcr");
        CU_ASSERT_PTR_NOT_NULL(vcrid);
        service = DIDDocument_GetService(doc, vcrid);
        CU_ASSERT_PTR_NOT_NULL(service);
        CU_ASSERT_EQUAL(1,DIDURL_Equals(vcrid, Service_GetId(service)));

        // Service not exist, should fail.
        DIDURL *notexistid = DIDURL_NewFromDid(did, "notExistService");
        ...
        CU_ASSERT_EQUAL(1,DIDURL_Equals(Service_GetId(services[0]), vcrid));
        DIDURL_Destroy(vcrid);

        CU_ASSERT_EQUAL(1, DIDDocument_SelectServices(doc, NULL, openid, services, sizeof(services)/sizeof(Service*)));
        CU_ASSERT_EQUAL(1,DIDURL_Equals(Service_GetId(services[0]), openid));
        ...
        CU_ASSERT_EQUAL(1,DIDURL_Equals(Service_GetId(services[0]), id));
        DIDURL_Destroy(id);
...
static void test_diddoc_add_service(void)
{
    ...
    for (j = 0; j < 4; j++) {
        ...
        DIDURL *id = DIDURL_NewFromDid(did, "vcr");
        CU_ASSERT_PTR_NOT_NULL(id1);
...
static void test_diddoc_remove_service(void)
{
    ...
    for (j = 0; j < 4; j++) {
        ...
        // Service not exist, should fail.
        DIDURL *id = DIDURL_NewFromDid(did, "notExistService");
        CU_ASSERT_PTR_NOT_NULL(id);
...
static void test_diddoc_remove_proof(void)
{
    ...
    for (j = 0; j < 4; j++) {
        ...
        CU_ASSERT_EQUAL(1, DIDDocument_IsValid(document));
        CU_ASSERT_EQUAL(1,DIDURL_Equals(creater, DIDDocument_GetProofCreater(document, 0)));
