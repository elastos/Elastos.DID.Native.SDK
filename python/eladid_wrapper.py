...
def __get_gc_obj(src, release_name=None):
    # release_name = None
...
def _str_call(name, *args, release_name=None):
    ...
    if not result:
        raise ElaDIDException(ElaError.get(name))
    return ffi.string(__get_gc_obj(result, release_name=release_name)).decode()
...
def _obj_call(name, *args, release_name=None):
    ...
    if not result:
        raise ElaDIDException(ElaError.get(name))
    return __get_gc_obj(result, release_name=release_name)
...
class Issuer:
    ...
    @staticmethod
    def create(did: DID, sign_key: DIDURL, store: 'DIDStore'):
        return _obj_call('Issuer_Create', did.did, sign_key.url, store.store, release_name='Issuer_Destroy')
...
class DIDStore:
    ...
    def list_dids(self, filter_has_private_keys=1) -> []:
        """
        :return: list[DID]
        """
        dids = []
        ...
        _int_call('DIDStore_ListDIDs', self.store, filter_has_private_keys, ListDIDsCallback, ffi.NULL)
        return dids
